...
public final class ThreadsBarneshut {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (child != null) {
				...
				if (ch instanceof OctTreeLeafNodeData) {
					body[curr++] = (OctTreeLeafNodeData) ch; // sort bodies in
																// tree order
																// (approximation
																// of putting
																// nearby nodes
	...
	public static long run(String args[]) {
		...
		for (int step1 = 0; step1 < ntimesteps; step1++) { // time-step the system
			...
			final Node<OctTreeNodeData> root = octree.createNode(new OctTreeNodeData(centerx, centery, centerz)); // create
																											// the
																											// tree's
																											// root
			octree.addNode(root);
			...
			for (int i = 0; i < nbodies; i++) {
				Insert(octree, root, body[i], radius); // grow the tree by
														// inserting each body
			}
			curr = 0;
			...
			Thread[] t = new Thread[body.length];
			
			for (int i = 0;i<body.length;i++) {
				final OctTreeLeafNodeData n = body[i];
				// compute the acceleration of each body (consumes most of the
				// total runtime)
			...
			for (int i = 0; i < nbodies; i++) {
				body[i].Advance(dthf, dtime); // advance the position and
		...
		if (isFirstRun) {
			...
			for (int i = 0; i < nbodies; i++) {
				System.out.println(df.format(body[i].posx) + " " + df.format(body[i].posy) + " "
						+ df.format(body[i].posz)); // print result
			}
			System.out.println();
		}
