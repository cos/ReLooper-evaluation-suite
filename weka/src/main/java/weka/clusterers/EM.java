...
public class EM extends RandomizableDensityBasedClusterer implements
	...
	public static final List statistics = new ArrayList();
	...
	private void EM_Init(final Instances inst) throws Exception {
		...
		// SimpleKMeans[] skArrs = new SimpleKMeans[10];
		// long startArrLoop = System.nanoTime();
		// for (i = 0; i < 10; i++) {
		// SimpleKMeans sk = new SimpleKMeans();
		// //sk.setSeed(seeds[i]);
		...
		// initialize with best k-means solution
		m_num_clusters = bestK[0].numberOfClusters();
		m_weights = new double[inst.numInstances()][m_num_clusters];
		m_model = new DiscreteEstimator[m_num_clusters][m_num_attribs];
		m_modelNormal = new double[m_num_clusters][m_num_attribs][3];
		m_priors = new double[m_num_clusters];
		Instances centers = bestK[0].getClusterCentroids();
		Instances stdD = bestK[0].getClusterStandardDevs();
		int[][][] nominalCounts = bestK[0].getClusterNominalCounts();
		int[] clusterSizes = bestK[0].getClusterSizes();
	...
	// private SimpleKMeans[] skArrs;

	private double totalArrTime = 0.0;

	private double totalParTime = 0.0;
