...
public class Capabilities implements Cloneable, Serializable, RevisionHandler {
	...
	public Capabilities(CapabilitiesHandler owner) {
		...
		m_MinimumNumberInstancesTest = Boolean.parseBoolean(PROPERTIES.getProperty("MinimumNumberInstancesTest", "true"))
		    && m_Test;
	}
	...
	public boolean test(Attribute att, boolean isClass) {
		...
		case Attribute.NOMINAL:
			...
			if (att.numValues() == 1) {
				...
				m_FailReason = new UnsupportedAttributeTypeException(createMessage("Cannot handle multi-valued nominal "
				    + errorStr + "!"));
				result = false;
			}
			break;
		...
		default:
			m_FailReason = new UnsupportedAttributeTypeException(createMessage("Cannot handle unknown attribute type '"
			    + att.type() + "'!"));
			result = false;
		}
	...
	public boolean test(Instances data, int fromIndex, int toIndex) {
						...
						if (hasClass < getMinimumNumberInstances()) {
							m_FailReason = new WekaException(
							    createMessage("Not enough training instances with class labels (required: "
							        + getMinimumNumberInstances() + ", provided: " + hasClass + ")!"));
							return false;
						}
					}
				}
		...
		if (m_MinimumNumberInstancesTest) {
			if (data.numInstances() < getMinimumNumberInstances()) {
				m_FailReason = new WekaException(createMessage("Not enough training instances (required: "
				    + getMinimumNumberInstances() + ", provided: " + data.numInstances() + ")!"));
				return false;
			}
		}
		...
		if (handles(Capability.ONLY_MULTIINSTANCE)) {
			...
			if (!data.attribute(0).isNominal() || !data.attribute(1).isRelationValued()
