...
public class AppDemo extends Universal {
	...
	public static ParallelArray<Object> tasks;
	...
	public void runParallel() {
		...
		tasks.apply(new Procedure<Object>() {

			@Override
			public void op(Object b) {
	...
	private void initTasks(int nRunsMC) {
		tasks = ParallelArray.createUsingHandoff(new Object[nRunsMC], ParallelArray.defaultExecutor());

		tasks.replaceWithMappedIndex(new IntToObject<Object>() {
			@Override
			public Object op(int i) {
...
class AppDemoThread implements Runnable {
	...
	public void run() {
		...
		for (int iRun = ilow; iRun < iupper; iRun++) {
			ps = new PriceStock();
			ps.setInitAllTasks(AppDemo.initAllTasks);
			ps.setTask(AppDemo.tasks.getArray()[iRun]);
			ps.run();
			AppDemo.results.addElement(ps.getResult());
		}
